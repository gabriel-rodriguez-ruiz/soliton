...
plt.tight_layout()
probability_density_right = probability_density[index][:, L_x//2]/np.sum(probability_density[index][:, L_x//2])

xi = 1/2  #m_0/v
y_analytical = np.linspace(1, L_y+1, 10000)
probability_density_analytical = np.sqrt(xi)*np.exp(-xi*np.abs(y_analytical-y_s))

fig, ax = plt.subplots()
ax.plot(y, probability_density_right, "o")
ax.plot(y_analytical, probability_density_analytical)
